classdef AlongTrackSimulator < AlongTrackSimulatorBase
    ...
    properties
        ...
        %
        % A containers.Map (string -> struct) describing orbit geometry and metadata
    ...
    methods
        ...
        function [lat,lon,time] = groundTrackForMissionWithName(self,mission,options)
            ...
            %
            % - Topic: AlongTrack — Ground tracks
            ...
            % - Returns lat: double column vector — geodetic latitude [deg]
            % - Returns lon: double column vector — longitude [deg] in [-180,180] or [0,360] depending on implementation
        ...
        function [lat,lon,time] = repeatGroundTrackForMissionWithName(self,mission)
            ...
            % complete repeat period (repeat_cycle days).
            %
            % - Topic: AlongTrack — Ground tracks
        ...
        function T = orbitalPeriodForMissionWithName(self,mission)
            % Return the Keplerian orbital period for a mission.
            %
            % - Topic: AlongTrack — Orbital periods
        ...
        function T = repeatCycleForMissionWithName(self,mission)
            ...
            %
            % - Topic: AlongTrack — Repeat cycles
        ...
        function alongtrack = projectedPointsForMissionWithName(self,missionName,requiredOptions,options)
            ...
            %
            % Computes the ground track for the requested mission, filters points to a
            % latitude/longitude bounding box, projects to (x,y) using a Transverse Mercator
            % projection about requiredOptions.lon0, and returns points within a rectangle
            % of size (Lx,Ly). The output time vector is sorted ascending.
            %
            % - Topic: AlongTrack — Along-track sampling
            ...
            % - Parameter requiredOptions.Lx: double — box width [m] or [km] consistent with projection implementation
            % - Parameter requiredOptions.Ly: double — box height
            ...
            % - Returns alongtrack: struct with fields
            ...
            %   - repeatCycle: double — repeat cycle duration [s] (if applicable)
            arguments
                self AlongTrackSimulator
                missionName string
                requiredOptions.Lx
        ...
        function alongtrack = projectedPointsForRepeatMissionWithName(self,missionName,requiredOptions,options)
            ...
            %
            % Like projectedPointsForMissionWithName, but uses repeatGroundTrackForMissionWithName
            % to generate points spanning a full repeat cycle, then filters and projects them.
            %
            % - Topic: AlongTrack — Along-track sampling
            % - Declaration: alongtrack = projectedPointsForRepeatMissionWithName(self,missionName,requiredOptions,options)
            % - Parameter self: AlongTrackSimulator
            % - Parameter missionName: string — mission key
            % - Parameter requiredOptions.Lx: double — box width
            % - Parameter requiredOptions.Ly: double — box height
            % - Parameter requiredOptions.lat0: double — box reference latitude [deg]
            % - Parameter requiredOptions.lon0: double — central meridian for projection [deg]
            % - Parameter options.origin: {'lower-left','center'} (default 'lower-left') — coordinate origin convention for returned (x,y)
            % - Returns alongtrack: struct with fields x, y, t, repeatCycle (see projectedPointsForMissionWithName)
            arguments
                self AlongTrackSimulator
                missionName string
                requiredOptions.Lx
    ...
    methods (Static)
        ...
        function outputGroup = wvmOutputGroupForRepeatMissionWithName(model, missionName)
            ...
            % to generate repeat-cycle projected track points, splits them into passes,
            % and packages them for downstream use.
            %
            % - Topic: AlongTrack — WaveVortexModel integration
            % - Declaration: outputGroup = wvmOutputGroupForRepeatMissionWithName(model, missionName)
            % - Parameter model: object — expected to have a field/property wvt with Lx, Ly, latitude
            % - Parameter missionName: string — mission key
            % - Returns outputGroup: struct — group containing per-pass track data (see implementation)
            % Create an AlongTrackSimulator instance.
            %
            % Loads the mission parameter catalog and (when applicable) adjusts mission
            % parameters so that repeat-cycle missions have an exact repeat by solving for
            % a semi-major axis consistent with the requested passes-per-cycle.
            %
            % - Topic: AlongTrack — Construction
            % - Declaration: self = AlongTrackSimulator()
            % - Returns self: AlongTrackSimulator
        ...
        function inclination = inclinationForExactRepeatForMission(mission)
            ...
            % Used for orbital period computations via Kepler's third law.
            %
            % - Topic: AlongTrack — Orbital mechanics
            % - Declaration: mu
